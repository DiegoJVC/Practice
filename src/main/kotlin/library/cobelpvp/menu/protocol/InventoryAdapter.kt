package library.cobelpvp.menu.protocol

import library.cobelpvp.menu.protocol.event.PlayerCloseInventoryEvent
import library.cobelpvp.menu.protocol.event.PlayerOpenInventoryEvent
...
class InventoryAdapter : PacketAdapter(Practice.getInstance(), PacketType.Play.Client.CLIENT_COMMAND, PacketType.Play.Client.CLOSE_WINDOW) {
    ...
    override fun onPacketReceiving(event: PacketEvent?) {
        ...
        if (packet.type === PacketType.Play.Client.CLIENT_COMMAND && packet.clientCommands.size() != 0 && packet.clientCommands.read(0) == EnumWrappers.ClientCommand.OPEN_INVENTORY_ACHIEVEMENT) {
            if (!currentlyOpen.contains(player.uniqueId)) {
                Bukkit.getScheduler().scheduleSyncDelayedTask(Practice.getInstance()) { Bukkit.getPluginManager().callEvent(
