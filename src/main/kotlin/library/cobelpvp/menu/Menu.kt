package library.cobelpvp.menu

import com.cobelpvp.practice.Practice
import library.cobelpvp.menu.protocol.Reflections
...
abstract class Menu {
    ...
    fun openMenu(player: Player) {
        if (async) {
            Practice.getInstance().server.scheduler.runTaskAsynchronously(
    ...
    private fun openCustomInventory(player: Player, inv: Inventory) {
        ...
        if (Bukkit.isPrimaryThread()) {
            ...
            Practice.getInstance().server.scheduler.runTaskLater(
                Practice.getInstance(), {
                if (OPEN_CUSTOM_INVENTORY_METHOD_LEGACY != null) {
                    OPEN_CUSTOM_INVENTORY_METHOD_LEGACY.invoke(player, inv, entityPlayer, 0)
                } else {
    ...
    fun update(player: Player) {
        ...
        runnable.runTaskTimer(Practice.getInstance(), 6L, 6L)
