...
import com.cobelpvp.practice.kit.listener.KitLoadListener;
...
public final class KitHandler {
    ...
    public static final String MONGO_COLLECTION_NAME = "usersKitsEdited";
    public static final int KITS_PER_TYPE = 1;
    @Getter @Setter

    private final Map<UUID, List<Kit>> kitData = new ConcurrentHashMap<>();

    public KitHandler() {
        Bukkit.getPluginManager().registerEvents(new KitLoadListener(), Practice.getInstance());
    }
    ...
    public Kit saveKit(Player player, Kit kit) {
        kitData.computeIfAbsent(player.getUniqueId(), i -> new ArrayList<>()).add(kit);
        saveKitsAsync(player);
    ...
    public void saveKitsAsync(Player player) {
        Bukkit.getScheduler().runTaskAsynchronously(Practice.getInstance(), () -> {
            ...
            List kitJson = (List) JSON.parse(Practice.getGson().toJson(kitData
                    .getOrDefault(player.getUniqueId(), ImmutableList.of())));
    ...
    public void loadKits(UUID playerUuid) {
        ...
        if (playerKits != null) {
            ...
            kitData.put(playerUuid, Practice.getGson().fromJson(JSON.serialize(kits), listKit));
        }
    }
