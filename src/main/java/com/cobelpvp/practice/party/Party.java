...
public final class Party {
    ...
    Party(UUID leader) {
        ...
        Practice.getInstance().getPartyHandler().updatePartyCache(leader, this);
    ...
    public void invite(Player target) {
        ...
        invites.add(invite);
        Bukkit.getScheduler().runTaskLater(Practice.getInstance(), () -> invites.remove(invite), PartyHandler.INVITE_EXPIRATION_SECONDS * 20);
    }
    ...
    public void join(Player player) {
        ...
        if (!PracticeValidation.canJoinParty(player, this)) {
            return;
        }
        ...
        player.sendMessage(ChatColor.YELLOW + "You have joined " + ChatColor.DARK_GREEN + leaderBukkit.getName() + ChatColor.YELLOW + "'s party.");
        message(ChatColor.DARK_GREEN + player.getName() + ChatColor.GREEN + " has joined your party.");
        members.add(player.getUniqueId());
        Practice.getInstance().getPartyHandler().updatePartyCache(player.getUniqueId(), this);
    ...
    public void leave(Player player) {
        ...
        Practice.getInstance().getPartyHandler().updatePartyCache(player.getUniqueId(), null);
        ...
        if (leader.equals(player.getUniqueId())) {
            ...
            message(ChatColor.DARK_GREEN + newLeader.getName() + ChatColor.GREEN + " has been randomly promoted to leader of your party.");
        }
    ...
    public void setLeader(Player player) {
        ...
        message(ChatColor.DARK_GREEN + player.getName() + ChatColor.GREEN + " has been promoted to leader of your party.");
        resetInventoriesDelayed();
    }

    public void disband() {
        Bukkit.getPluginManager().callEvent(new PartyDisbandEvent(this));
        Practice.getInstance().getPartyHandler().unregisterParty(this);
    ...
    public void resetInventoriesDelayed() {
        ...
        Bukkit.getScheduler().runTaskLater(Practice.getInstance(), this::resetInventoriesNow, InventoryUtils.RESET_DELAY_TICKS);
    }
