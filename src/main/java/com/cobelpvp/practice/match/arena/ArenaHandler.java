...
import com.cobelpvp.practice.match.arena.event.ArenaAllocatedEvent;
import com.cobelpvp.practice.match.arena.event.ArenaReleasedEvent;
import com.cobelpvp.practice.match.arena.listener.ArenaItemResetListener;
...
public final class ArenaHandler {
    ...
    public ArenaHandler() {
        Bukkit.getPluginManager().registerEvents(new ArenaItemResetListener(), Practice.getInstance());
        ...
        try {
            if (arenaInstancesFile.exists()) {
                try (Reader arenaInstancesReader = Files.newReader(arenaInstancesFile, Charsets.UTF_8)) {
                    ...
                    List<Arena> arenaList = Practice.getGson().fromJson(arenaInstancesReader, arenaListType);
            ...
            if (schematicsFile.exists()) {
                try (Reader schematicsFileReader = Files.newReader(schematicsFile, Charsets.UTF_8)) {
                    ...
                    List<ArenaSchematic> schematicList = Practice.getGson().fromJson(schematicsFileReader, schematicListType);
    ...
    public void saveSchematics() throws IOException {
        Files.write(
            Practice.getGson().toJson(schematics.values()),
    ...
    public void saveArenas() throws IOException {
        ...
        Files.write(
            Practice.getGson().toJson(allArenas),
