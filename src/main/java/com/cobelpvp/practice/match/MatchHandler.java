...
import com.cobelpvp.practice.match.listener.*;
...
public final class MatchHandler {
        ...
        for (MatchTeam team : teams) {
            for (UUID member : team.getAllMembers()) {
                ...
                if (isPlayingOrSpectatingMatch(memberPlayer)) {
                    throw new IllegalArgumentException(TeamsUUIDCache.name(member) + " is already in a match!");
                }
            }
        }
    ...
    public static boolean canUseSchematic(KitType kitType, ArenaSchematic schematic) {
        ...
        if (kitId.equals("FinalUHC")) return schematic.isFinalUHCOnly();
        ...
        if (schematic.isFinalUHCOnly()) return kitId.equals("FinalUHC");
